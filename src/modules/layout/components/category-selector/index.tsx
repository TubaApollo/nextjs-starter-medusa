...
import { ChevronRightIcon, ChevronDownIcon, ChevronLeftIcon, XMarkIcon } from "@heroicons/react/24/outline"
...
const CategorySelector: React.FC<CategorySelectorProps> = ({
  ...
  const [mobileOverlayStack, setMobileOverlayStack] = useState<Array<{
    categoryId: string
    level: number
    categories: HttpTypes.StoreProductCategory[]
    title: string
  ...
  const handleMobileCategoryClick = useCallback((category: HttpTypes.StoreProductCategory, level: number = 0) => {
    if (category.category_children && category.category_children.length > 0) {
      // Add new overlay to the stack
      setMobileOverlayStack(prev => [...prev, {
  ...
  const closeMobileOverlay = useCallback(() => {
    setMobileOverlayStack(prev => prev.slice(0, -1))
  }, [])

  const closeAllMobileOverlays = useCallback(() => {
    setMobileOverlayStack([])
  }, [])
  ...
  return (
    <div className="relative w-full flex justify-start">
      ...
      >
              ...
              {isMobile ? (
                <>
                  ...
                  <motion.div
                    ...
                    className="fixed top-20 left-4 right-4 bg-white shadow-2xl border border-gray-200 rounded-lg overflow-hidden z-50"
                    style={{ 
                      minHeight: '300px',
                      maxHeight: '70vh'
                    }}
                  >
                    <div className="p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-gray-800">Kategorien</h3>
                        <button
                          onClick={() => setIsOpen(false)}
                            ...
                            {hasSubcategories ? (
                              <button
                                ...
                                onClick={(e) => {
                                  e.preventDefault()
                                  e.stopPropagation()
                                  handleMobileCategoryClick(category, 0)
                                }}
                              >
                ...
                <div className="flex h-[400px] overflow-hidden">
                  ...
                  <div className="w-64 bg-gray-50 p-4 border-r border-gray-200 flex-shrink-0 h-full">
                    ...
                    <ul className="space-y-1">
                      {topLevelCategories.map((category) => {
                        const hasSubcategories = category.category_children && 
                                                category.category_children.length > 0 && 
                                                category.category_children.some(child => child.id)
        ...
        {/* Mobile Subcategory Overlays */}
        <AnimatePresence>
          {isMobile && mobileOverlayStack.map((overlay, index) => (
            <div key={`${overlay.categoryId}-${index}`}>
              {/* Backdrop - only for the topmost overlay */}
              {index === mobileOverlayStack.length - 1 && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                animate={{ opacity: 1, x: `${index * 20}px` }}
                exit={{ opacity: 0, x: '100%' }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="fixed top-0 bottom-0 w-80 bg-white shadow-2xl overflow-y-auto"
                style={{ 
                  right: `${index * 20}px`,
                  zIndex: 70 + index
                }}
              >
                <div className="p-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-gray-800">{overlay.title}</h3>
                    <button
                  ...
                  <div className="space-y-2">
                    {overlay.categories.map((subcategory: HttpTypes.StoreProductCategory) => {
                      // Use categoryMap to get the full category object with all nested data
                      const fullSubcategory = categoryMap.get(subcategory.id) || subcategory
                      const hasSubcategories = fullSubcategory.category_children && fullSubcategory.category_children.length > 0
                          ...
                          {hasSubcategories ? (
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault()
                                e.stopPropagation()
                                handleMobileCategoryClick(fullSubcategory, overlay.level + 1)
                              }}
                            >
